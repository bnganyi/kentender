...
class TestNationalFramework(FrappeTestCase):
	def tearDown(self):
		frappe.db.delete(DOCTYPE, {"name": ("like", "_KT_NF_%")})
		frappe.db.commit()
		super().tearDown()

	def test_valid_create(self):
		doc = frappe.get_doc({**_base_kwargs(), "name": "_KT_NF_001"})
		doc.insert()
		self.assertEqual(doc.name, "_KT_NF_001")

	def test_duplicate_business_id_blocked(self):
		frappe.get_doc({**_base_kwargs(), "name": "_KT_NF_DUP"}).insert()
		dup = frappe.get_doc({**_base_kwargs(), "name": "_KT_NF_DUP", "version_label": "v2"})
		self.assertRaises(frappe.DuplicateEntryError, dup.insert)

	def test_duplicate_framework_code_version_blocked(self):
		frappe.get_doc({**_base_kwargs(), "name": "_KT_NF_A", "version_label": "2026"}).insert()
		dup = frappe.get_doc({**_base_kwargs(), "name": "_KT_NF_B", "version_label": "2026"})
		self.assertRaises(frappe.DuplicateEntryError, dup.insert)

	def test_same_code_different_version_allowed(self):
		frappe.get_doc({**_base_kwargs(), "name": "_KT_NF_C1", "version_label": "2025"}).insert()
		doc2 = frappe.get_doc({**_base_kwargs(), "name": "_KT_NF_C2", "version_label": "2026"})
		doc2.insert()
	...
	def test_draft_can_edit(self):
		doc = frappe.get_doc({**_base_kwargs(), "name": "_KT_NF_ED"})
		doc.insert()

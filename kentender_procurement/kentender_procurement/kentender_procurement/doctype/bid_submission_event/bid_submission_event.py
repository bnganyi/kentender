...
"""Bid Submission Event — append-only bid lifecycle history (PROC-STORY-039)."""
...
class BidSubmissionEvent(Document):
	def validate(self):
		self._normalize_text_fields()
		self._set_display_label()
		if not self.is_new():
			frappe.throw(
				_("Bid Submission Event cannot be modified."),
				frappe.ValidationError,
				title=_("Append-only record"),
			)
		self._validate_bid_submission_exists()
		self._validate_actor_user_exists()
		self._validate_event_hash()

	def _normalize_text_fields(self) -> None:
		self.actor_role_context = _strip(self.actor_role_context)
		self.ip_address = _strip(self.ip_address)
		self.user_agent = _strip(self.user_agent)
	...
	def _validate_bid_submission_exists(self) -> None:
		...
		if not bn:
			return
		if not frappe.db.exists(BID_SUBMISSION_DOCTYPE, bn):
			frappe.throw(
	...
	def _validate_actor_user_exists(self) -> None:
	...
	def on_trash(self):
		frappe.throw(
			_("Bid Submission Event cannot be deleted."),
			frappe.ValidationError,
			title=_("Append-only record"),
		)

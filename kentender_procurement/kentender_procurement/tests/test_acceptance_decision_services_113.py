# Copyright (c) 2025, Midas and contributors
# License: MIT. See LICENSE

"""PROC-STORY-113: submit_acceptance_decision."""

import frappe
from frappe.tests.utils import FrappeTestCase
from frappe.utils import getdate, nowdate

from kentender.tests.test_procuring_entity import _ensure_test_currency, _make_entity, run_test_db_cleanup
from kentender_budget.tests.test_budget_control_period import _bcp

from kentender_procurement.services.acceptance_decision_services import submit_acceptance_decision

AD = "Award Decision"
AR = "Acceptance Record"
BOS = "Bid Opening Session"
BOR = "Bid Opening Register"
ES = "Evaluation Report"
EVAL = "Evaluation Session"
IMT = "Inspection Method Template"
IR = "Inspection Record"
IPL = "Inspection Parameter Line"
ISE = "Inspection Status Event"
PC = "Procurement Contract"
TENDER = "Tender"
PREFIX = "_KT_INSP113"


def _cleanup_113():
	frappe.flags.allow_inspection_status_event_delete = True
	ir_names = frappe.get_all(IR, filters={"business_id": ("like", f"{PREFIX}%")}, pluck="name") or []
	for irn in ir_names:
		for row in frappe.get_all(ISE, filters={"inspection_record": irn}, pluck="name") or []:
			frappe.delete_doc(ISE, row, force=True, ignore_permissions=True)
		for row in frappe.get_all(AR, filters={"inspection_record": irn}, pluck="name") or []:
			frappe.delete_doc(AR, row, force=True, ignore_permissions=True)
		for row in frappe.get_all(IPL, filters={"inspection_record": irn}, pluck="name") or []:
			frappe.delete_doc(IPL, row, force=True, ignore_permissions=True)
	for row in ir_names:
		frappe.delete_doc(IR, row, force=True, ignore_permissions=True)
	for row in frappe.get_all(IMT, filters={"name": ("like", f"{PREFIX}%")}, pluck="name") or []:
		frappe.delete_doc(IMT, row, force=True, ignore_permissions=True)
	for row in frappe.get_all(PC, filters={"business_id": ("like", f"{PREFIX}%")}, pluck="name") or []:
		frappe.delete_doc(PC, row, force=True, ignore_permissions=True)
	for row in frappe.get_all(AD, filters={"business_id": ("like", f"{PREFIX}%")}, pluck="name") or []:
		frappe.delete_doc(AD, row, force=True, ignore_permissions=True)
	for row in frappe.get_all(ES, filters={"business_id": ("like", f"{PREFIX}%")}, pluck="name") or []:
		frappe.delete_doc(ES, row, force=True, ignore_permissions=True)
	for row in frappe.get_all(EVAL, filters={"business_id": ("like", f"{PREFIX}%")}, pluck="name") or []:
		frappe.delete_doc(EVAL, row, force=True, ignore_permissions=True)
	for row in frappe.get_all(BOR, filters={"business_id": ("like", f"{PREFIX}%")}, pluck="name") or []:
		frappe.delete_doc(BOR, row, force=True, ignore_permissions=True)
	for row in frappe.get_all(BOS, filters={"business_id": ("like", f"{PREFIX}%")}, pluck="name") or []:
		frappe.delete_doc(BOS, row, force=True, ignore_permissions=True)
	for tn in frappe.get_all(TENDER, filters={"name": ("like", f"{PREFIX}%")}, pluck="name") or []:
		frappe.delete_doc(TENDER, tn, force=True, ignore_permissions=True)
	for row in frappe.get_all("Bid Submission", filters={"business_id": ("like", f"{PREFIX}%")}, pluck="name") or []:
		frappe.delete_doc("Bid Submission", row, force=True, ignore_permissions=True)
	frappe.db.delete("Budget Control Period", {"name": ("like", f"{PREFIX}%")})
	frappe.db.delete("Procuring Entity", {"entity_code": f"{PREFIX}_PE"})
	frappe.flags.allow_inspection_status_event_delete = False


class TestAcceptanceDecisionServices113(FrappeTestCase):
	def setUp(self):
		super().setUp()
		self.currency = _ensure_test_currency()
		run_test_db_cleanup(_cleanup_113)
		self.entity = _make_entity(f"{PREFIX}_PE").insert()
		self.period = _bcp(f"{PREFIX}_BCP", self.entity.name).insert()

	def tearDown(self):
		run_test_db_cleanup(_cleanup_113)
		super().tearDown()

	def _minimal_award_and_pc(self, suffix: str):
		t = frappe.get_doc(
			{
				"doctype": TENDER,
				"name": f"{PREFIX}_T{suffix}",
				"business_id": f"{PREFIX}_T{suffix}-BIZ",
				"title": "INSP113",
				"tender_number": f"{PREFIX}_TN{suffix}",
				"workflow_state": "Draft",
				"status": "Draft",
				"approval_status": "Draft",
				"origin_type": "Manual",
				"procuring_entity": self.entity.name,
				"currency": self.currency,
			}
		).insert(ignore_permissions=True)
		s = frappe.get_doc(
			{
				"doctype": BOS,
				"business_id": f"{PREFIX}_S{suffix}",
				"tender": t.name,
				"procuring_entity": self.entity.name,
				"status": "Draft",
				"workflow_state": "Draft",
			}
		).insert(ignore_permissions=True)
		r = frappe.get_doc(
			{
				"doctype": BOR,
				"business_id": f"{PREFIX}_R{suffix}",
				"tender": t.name,
				"bid_opening_session": s.name,
				"status": "Draft",
			}
		).insert(ignore_permissions=True)
		e = frappe.get_doc(
			{
				"doctype": EVAL,
				"business_id": f"{PREFIX}_E{suffix}",
				"tender": t.name,
				"procuring_entity": self.entity.name,
				"status": "Draft",
				"workflow_state": "Draft",
				"opening_session": s.name,
				"opening_register": r.name,
				"evaluation_mode": "Two Envelope",
				"conflict_clearance_status": "Pending",
			}
		).insert(ignore_permissions=True)
		rep = frappe.get_doc(
			{
				"doctype": ES,
				"business_id": f"{PREFIX}_ER{suffix}",
				"evaluation_session": e.name,
				"tender": t.name,
				"status": "Draft",
			}
		).insert(ignore_permissions=True)
		b = frappe.get_doc(
			{
				"doctype": "Bid Submission",
				"business_id": f"{PREFIX}_B{suffix}",
				"tender": t.name,
				"supplier": f"S-{suffix}",
				"tender_lot_scope": "Whole Tender",
				"procuring_entity": self.entity.name,
				"procurement_method": "Open National Tender",
				"status": "Draft",
				"workflow_state": "Draft",
				"submission_version_no": 1,
				"quoted_total_amount": 5000,
			}
		).insert(ignore_permissions=True)
		ad = frappe.get_doc(
			{
				"doctype": AD,
				"business_id": f"{PREFIX}_AD{suffix}",
				"tender": t.name,
				"evaluation_session": e.name,
				"evaluation_report": rep.name,
				"decision_justification": "113 tests.",
				"recommended_bid_submission": b.name,
				"recommended_supplier": f"S-{suffix}",
				"recommended_amount": 5000,
				"approved_bid_submission": b.name,
				"approved_supplier": f"S-{suffix}",
				"approved_amount": 5000,
				"currency": self.currency,
			}
		).insert(ignore_permissions=True)
		pc = frappe.get_doc(
			{
				"doctype": PC,
				"business_id": f"{PREFIX}_PC{suffix}",
				"contract_title": "113 contract",
				"award_decision": ad.name,
				"tender": t.name,
				"evaluation_session": e.name,
				"approved_bid_submission": b.name,
				"supplier": f"S-{suffix}",
				"procuring_entity": self.entity.name,
				"contract_value": 5000,
				"currency": self.currency,
				"contract_start_date": getdate(nowdate()),
			}
		).insert(ignore_permissions=True)
		return pc

	def _template(self, suf: str):
		c = f"{PREFIX}_{suf}"
		return frappe.get_doc(
			{
				"doctype": IMT,
				"template_code": c,
				"template_name": "Tpl",
				"inspection_domain": "General",
				"applicable_contract_type": "Goods",
				"inspection_method_type": "Parameter Testing",
				"default_pass_logic": "All Mandatory Pass",
				"active": 1,
			}
		).insert(ignore_permissions=True)

	def _completed_ir(self, pc, biz: str, tpl, *, inspection_result: str = "Pass"):
		return frappe.get_doc(
			{
				"doctype": IR,
				"business_id": biz,
				"contract": pc.name,
				"inspection_scope_type": "Contract Wide",
				"inspection_title": "113 inspection",
				"inspection_method_type": "Parameter Testing",
				"inspection_method_template": tpl.name,
				"procuring_entity": self.entity.name,
				"status": "Completed",
				"workflow_state": "Completed",
				"inspection_result": inspection_result,
			}
		).insert(ignore_permissions=True)

	def test_KT_INSP113_accepted_creates_record_and_event(self):
		pc = self._minimal_award_and_pc("A")
		tpl = self._template("TPLA")
		ir = self._completed_ir(pc, f"{PREFIX}_IR1", tpl, inspection_result="Pass")
		frappe.get_doc(
			{
				"doctype": IPL,
				"inspection_record": ir.name,
				"parameter_code": "M1",
				"parameter_name": "Mandatory",
				"parameter_category": "Dimensional",
				"tolerance_type": "MinMax",
				"expected_min_value": 0.0,
				"expected_max_value": 10.0,
				"mandatory_for_acceptance": 1,
				"status": "Passed",
			}
		).insert(ignore_permissions=True)
		out = submit_acceptance_decision(
			ir.name,
			{
				"business_id": f"{PREFIX}_ACC_OK",
				"acceptance_decision": "Accepted",
				"decision_reason": "All good.",
				"event_summary": "Signed off.",
			},
		)
		self.assertTrue(out["name"])
		self.assertEqual(out["inspection_acceptance_status"], "Accepted")
		ir2 = frappe.get_doc(IR, ir.name)
		self.assertEqual(ir2.acceptance_record, out["name"])
		ev = frappe.get_all(
			ISE,
			filters={"inspection_record": ir.name, "event_type": "AcceptanceRecorded"},
			pluck="name",
		)
		self.assertEqual(len(ev), 1)
		self.assertEqual(frappe.db.get_value(ISE, ev[0], "related_acceptance_record"), out["name"])

	def test_KT_INSP113_rejected_outcome(self):
		pc = self._minimal_award_and_pc("B")
		tpl = self._template("TPLB")
		ir = self._completed_ir(pc, f"{PREFIX}_IR2", tpl, inspection_result="Fail")
		out = submit_acceptance_decision(
			ir.name,
			{"business_id": f"{PREFIX}_ACC_REJ", "acceptance_decision": "Rejected", "decision_reason": "Failed inspection."},
		)
		self.assertEqual(out["inspection_acceptance_status"], "Rejected")
		ar = frappe.get_doc(AR, out["name"])
		self.assertEqual(ar.payment_eligibility_signal_status, "Not Eligible")

	def test_KT_INSP113_conditional_partial_when_mandatory_unsatisfied(self):
		pc = self._minimal_award_and_pc("C")
		tpl = self._template("TPLC")
		ir = self._completed_ir(pc, f"{PREFIX}_IR3", tpl, inspection_result="Pass")
		frappe.get_doc(
			{
				"doctype": IPL,
				"inspection_record": ir.name,
				"parameter_code": "M2",
				"parameter_name": "Pending mandatory",
				"parameter_category": "Dimensional",
				"tolerance_type": "MinMax",
				"expected_min_value": 0.0,
				"expected_max_value": 10.0,
				"mandatory_for_acceptance": 1,
				"status": "Pending",
			}
		).insert(ignore_permissions=True)
		out = submit_acceptance_decision(
			ir.name,
			{
				"business_id": f"{PREFIX}_ACC_PART",
				"acceptance_decision": "Conditional",
				"decision_reason": "Accept subject to retest of M2.",
			},
		)
		self.assertEqual(out["inspection_acceptance_status"], "Conditional")

	def test_KT_INSP113_blocks_accepted_when_mandatory_not_satisfied(self):
		pc = self._minimal_award_and_pc("D")
		tpl = self._template("TPLD")
		ir = self._completed_ir(pc, f"{PREFIX}_IR4", tpl, inspection_result="Pass")
		frappe.get_doc(
			{
				"doctype": IPL,
				"inspection_record": ir.name,
				"parameter_code": "M3",
				"parameter_name": "Bad",
				"parameter_category": "Dimensional",
				"tolerance_type": "MinMax",
				"expected_min_value": 0.0,
				"expected_max_value": 10.0,
				"mandatory_for_acceptance": 1,
				"status": "Failed",
			}
		).insert(ignore_permissions=True)
		self.assertRaises(
			frappe.ValidationError,
			submit_acceptance_decision,
			ir.name,
			{"business_id": f"{PREFIX}_ACC_BLOCK", "acceptance_decision": "Accepted"},
		)

	def test_KT_INSP113_blocks_when_inspection_not_completed(self):
		pc = self._minimal_award_and_pc("E")
		tpl = self._template("TPLE")
		ir = frappe.get_doc(
			{
				"doctype": IR,
				"business_id": f"{PREFIX}_IR5",
				"contract": pc.name,
				"inspection_scope_type": "Contract Wide",
				"inspection_title": "Draft",
				"inspection_method_type": "Parameter Testing",
				"inspection_method_template": tpl.name,
				"procuring_entity": self.entity.name,
				"status": "Draft",
				"workflow_state": "Draft",
				"inspection_result": "Pass",
			}
		).insert(ignore_permissions=True)
		self.assertRaises(
			frappe.ValidationError,
			submit_acceptance_decision,
			ir.name,
			{"business_id": f"{PREFIX}_ACC_EARLY", "acceptance_decision": "Accepted"},
		)

	def test_KT_INSP113_blocks_accepted_when_result_not_pass(self):
		pc = self._minimal_award_and_pc("F")
		tpl = self._template("TPLF")
		ir = self._completed_ir(pc, f"{PREFIX}_IR6", tpl, inspection_result="Pass With Observations")
		self.assertRaises(
			frappe.ValidationError,
			submit_acceptance_decision,
			ir.name,
			{"business_id": f"{PREFIX}_ACC_OBS", "acceptance_decision": "Accepted"},
		)

	def test_KT_INSP113_deferred_maps_to_pending_acceptance_status(self):
		pc = self._minimal_award_and_pc("G")
		tpl = self._template("TPLG")
		ir = self._completed_ir(pc, f"{PREFIX}_IR7", tpl, inspection_result="Pass")
		out = submit_acceptance_decision(
			ir.name,
			{"business_id": f"{PREFIX}_ACC_DEF", "acceptance_decision": "Deferred", "decision_reason": "Awaiting docs."},
		)
		self.assertEqual(out["inspection_acceptance_status"], "Pending")

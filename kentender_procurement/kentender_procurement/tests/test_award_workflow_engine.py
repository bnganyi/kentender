...
"""WF-012: Award Decision workflow via kentender.workflow_engine."""
...
def _cleanup_awdwf():
	pol_code = f"{PREFIX}_POL"
	tpl_code = f"{PREFIX}_TPL"
...
def _ensure_award_policy(*, n_steps: int) -> None:
	pol_code = f"{PREFIX}_POL"
	tpl_code = f"{PREFIX}_TPL"
	if frappe.db.exists("KenTender Workflow Policy", {"policy_code": pol_code}):
		return
	...
	tpl = frappe.get_doc(
		{
			...
			"template_name": f"Award WF test {n_steps}-step",
			"object_type": AD,
			"steps": steps,
		}
	)
...
class TestAwardWorkflowEngine(FrappeTestCase):
	def setUp(self):
		super().setUp()
		self.currency = _ensure_test_currency()
		run_test_db_cleanup(_cleanup_awdwf)
	...
	def _chain_and_award(self, suffix: str):
		t = frappe.get_doc(
			{
				"doctype": TENDER,
	...
	def test_single_step_submit_approve(self):
		_ensure_award_policy(n_steps=1)
		ad = self._chain_and_award("1")
		submit_award_for_approval(ad.name)
		ad.reload()
		self.assertEqual(ad.workflow_state, "Pending Approval")
		final_approve_award(ad.name)
		ad.reload()
		self.assertEqual(ad.workflow_state, "Approved")

	def test_two_step_recommend_then_final(self):
		_ensure_award_policy(n_steps=2)
		ad = self._chain_and_award("2")
		submit_award_for_approval(ad.name)
		ad.reload()
		self.assertEqual(ad.workflow_state, "In Progress")
		approve_award_step(ad.name, workflow_step="S1", decision_level="L1", action_type="Recommend")
		ad.reload()
		self.assertEqual(ad.workflow_state, "Pending Approval")
		final_approve_award(ad.name)
		ad.reload()
	...
	def test_reject_after_submit(self):
		_ensure_award_policy(n_steps=1)
		ad = self._chain_and_award("3")
		submit_award_for_approval(ad.name)
		reject_award(ad.name, comments="No")
		ad.reload()
		self.assertEqual(ad.workflow_state, "Rejected")

	def test_direct_workflow_save_blocked(self):
		_ensure_award_policy(n_steps=1)
		ad = self._chain_and_award("4")
		submit_award_for_approval(ad.name)
		ad.reload()
		with self.assertRaises(frappe.ValidationError):
			ad.workflow_state = "Approved"
			ad.save(ignore_permissions=True)

	def test_direct_workflow_save_allowed_in_mutation_context(self):
		_ensure_award_policy(n_steps=1)
		ad = self._chain_and_award("5")
		submit_award_for_approval(ad.name)
		ad.reload()
		with workflow_mutation_context():
			ad.workflow_state = "Rejected"
			ad.status = "Rejected"

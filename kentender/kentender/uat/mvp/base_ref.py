...
def _upsert_doc(doctype: str, name_field: str, name: str, fields: dict[str, Any]) -> str:
	"""Insert or update by primary name (autoname = field)."""
...
def load_base_ref(ds: dict[str, Any]) -> dict[str, Any]:
	"""BASE-REF: entity, departments, funding source, category, method, document type."""
	...
	dtr = ds.get("document_type_registry") or {}
	_upsert_doc(
		"Document Type Registry",
		"document_type_code",
		dtr.get("document_type_code"),
		{
			"document_type_name": dtr.get("document_type_name"),
			"module_scope": dtr.get("module_scope") or "Procurement",

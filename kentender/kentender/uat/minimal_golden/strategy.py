...
def load_strategy(ds: dict[str, Any], *, procuring_entity: str) -> dict[str, Any]:
	"""National + entity strategy chain for Minimal Golden (GOLD-SEED-003)."""
	...
	_upsert_business_doc(
		...
		b_tgt,
		{
			...
			"target_value_numeric": tg.get("target_value_numeric") or 0,
...
def assert_strategy_context_for_pr(ds: dict[str, Any]) -> dict[str, Any]:
	"""Return the same keys as :func:`load_strategy` after verifying GOLD strategy docs exist."""
	st = ds.get("strategy") or {}

	def _bid(section: str) -> str:
		b = (st.get(section) or {}).get("name")
		if not b:
			frappe.throw(
				_("Minimal Golden dataset is missing strategy.{0}.name").format(section),
				frappe.ValidationError,
			)
		return b

	b_nf = _bid("national_framework")
	b_pl = _bid("national_pillar")
	b_ob = _bid("national_objective")
	b_esp = _bid("entity_strategic_plan")
	b_pg = _bid("program")
	b_sg = _bid("sub_program")
	b_ind = _bid("output_indicator")
	b_tgt = _bid("performance_target")
	checks = (
		("National Framework", b_nf),
		("National Pillar", b_pl),
		("National Objective", b_ob),
		("Entity Strategic Plan", b_esp),
		("Strategic Program", b_pg),
		("Strategic Sub Program", b_sg),
		("Output Indicator", b_ind),
		("Performance Target", b_tgt),
	)
